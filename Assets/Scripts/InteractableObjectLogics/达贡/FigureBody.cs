...
public class FigureBody : MonoBehaviour
{
    public int itemId = 511; // 道具ID，由用户填写
    ...
    void Awake()
    {
        ...
        else
        {
            ...
            Debug.Log($"[FigureBody] 首次注册道具点到字典，状态: false, 关卡: {GameLevelManager.Instance.gameLevelType}, 坐标: {this.transform.position}, ItemId: {itemId}");
        }
    }
    ...
    private void CollectItem()
    {
        ...
        Debug.Log($"[FigureBody] 道具被收集，状态已写入字典: true, 关卡: {GameLevelManager.Instance.gameLevelType}, 坐标: {this.transform.position}, ItemId: {itemId}");
        ...
        UIManager.Instance.ShowPanel<AVGPanel>().InitAVG(2309);
