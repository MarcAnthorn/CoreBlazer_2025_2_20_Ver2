...
public class SecondFloorDiffer : MonoBehaviour
{
    ...
    void Update()
    {
        if(isInAera && !isTriggered && ItemManager.Instance.CheckIfItemExist(515))
        {
            isTriggered = true;
            EventHub.Instance.EventTrigger("Item515OnEffect");
        }
    }
    ...
    void OnTriggerExit2D(Collider2D collision)
    {
        ...
        {
            isInAera = false;
            EventHub.Instance.EventTrigger("Item515OnEffectOff");
            isTriggered = false;
        }
    }
